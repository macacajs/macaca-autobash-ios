...
# Upload the zip file to intranet CDN
outputTitle "Uploading binary version $UPDATED_VERSION..."
UPLOAD_RESULT=$(basefileUpload \
"${PROJECT_NAME}.zip" "$UPDATED_VERSION" \
| grep -o "\"http.*\.zip\"")
[ -z "$UPLOAD_RESULT" ] && exit 10
echo "Uploaded at $UPLOAD_RESULT."
...
ESCAPED_UPLOAD_RESULT=$(echo "${UPLOAD_RESULT}" | \
sed -e 's/\//\\\//g' | \
sed -e 's/\./\\\./g')
